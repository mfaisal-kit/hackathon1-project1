"""
Enhanced ingestion script to populate Qdrant database with multiple book pages
"""
import os
import asyncio
import hashlib
...
class EnhancedBookIngestor:
    ...
    async def discover_urls(self, base_url: str) -> List[str]:
        """Discover all URLs from the Docusaurus website"""
        ...
        try:
            ...
            for link in soup.find_all('a', href=True):
                ...
                # Only include internal links that are part of the documentation
                if (self._is_same_domain(full_url, base_url) and 
                    ('/docs/' in full_url or '/modules/' in full_url or full_url == base_url)):
                    urls.append(full_url)
            
            # Also try to get from sitemap if available
            ...
            filtered_urls = [url for url in unique_urls 
                           ...
                           not url.endswith(('.jpg', '.jpeg', '.png', '.gif', '.pdf', '.zip', '.css', '.js'))]
            
            logger.info(f"Discovered {len(filtered_urls)} URLs")
            return filtered_urls[:20]  # Limit to first 20 URLs to avoid overwhelming
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for loc in soup.find_all('loc'):
                    ...
                    if '/docs/' in url or '/modules/' in url:
                        urls.append(url)
    ...
    async def run_ingestion(self, base_url: str):
        ...
        # Discover URLs
        urls = await self.discover_urls(base_url)
        ...
        logger.info(f"Enhanced ingestion pipeline completed! Processed {total_processed} URLs with {total_chunks} total content chunks.")
...
async def main():
    ...
    ingestor = EnhancedBookIngestor()
    
    # Run full ingestion
    await ingestor.run_ingestion(book_url)
    
    logger.info("Enhanced ingestion pipeline completed!")
